...
const chekRegstr1 = function(agrn1, agrn2){
    
for(let i = 0; i < users.length; i++){
    if(users[i]['registrationDate'] == agrn1){
console.log(users[i])
    }
    else if(users[i]['registrationDate'] == agrn2){
        console.log(users[i])
    }
}
...
chekRegstr1("12.10.2021", "10.10.2021")
...
let chekid = Array.from(new Set(id.map(item => JSON.stringify(item)))).map(item => JSON.parse(item));
console.log(chekid)
